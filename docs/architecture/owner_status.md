...
```json
{
  ...
  "restart_count": 0,
  "terminated": {
    "reason": "Completed" | "GraphNodeFault" | "ExternalProcessExited"
              | "LivenessFailure" | "Evicted" | "FluxorReservationInvalid",
    "exit_code": 0,
    "signal": null,
    "finished_at": "<rfc3339>"
  },
  "waiting_reason": "FluxorReserving" | "ActivationBackOff" | null
}
```

`terminated` is present only when `phase == "Terminated"`; `waiting_reason`
only when `phase == "Activating"`.

The two reason lists are **closed vocabularies**. The reader deserializes them
into Rust enums, so a value outside the set cannot reach a consumer — a
writer/reader version skew fails the parse instead of propagating a string
nothing downstream understands.

Reasons the Linux runtime emits today:
...
`ExternalProcessExited`, `Evicted` and `FluxorReservationInvalid` are reserved
for the paths that can produce them — the external-process bridge, eviction,
and reservation loss respectively — and are in the vocabulary so those paths
can land without a format change.
...
- **Co-resident attribution.** With several owners in one runtime, a fault in a
  ...
  untouched. [`examples/owner_status/linux_two_pods.yaml`](../../examples/owner_status/linux_two_pods.yaml)
