...
Fluxor organises systems around capabilities and graphs, not around
...
physically packaged. A button is usually modelled as belonging to a
...
voice-assistant action: separate categories only because they arrive
through separate transports. In Fluxor they are all providers of the same semantic
...
The same generalisation applies to keyboards, displays, sensors, and
...
machine can provide the text-input capability for another. A
...
## One Execution Model, Many Workloads

The same execution primitives (modules, typed channels, topological
walk, stream time, bounded arenas) express workloads across a wide
range:
...
- storage services backed by NVMe
...
explicit timing and validated before deployment, stayed the same.
...
- platform code owns low-level register and boot behaviour
...
operating system becomes less central as an organisational unit. It
...
In this model (a direction, not a shipped surface), those become
capabilities. An `object_detection` capability emits
detection events and accepts configuration commands. Its
implementation is internal: a quantised classifier on a Pico, an
NPU-accelerated model on an aarch64 board, a poll-mode pipeline on a
server, a recorded stream replayed in place of live capture. The consumer binds to
...
- modules cross targets unchanged: the same foundation modules
  (`ip`, `fat32`, `tls`, `dns`) are wired into MCU, bare-metal Pi 5,
  and Linux example configs
- remote channels and protocol surfaces extend the channel model
  across process and network boundaries

These are working primitives; the capability-centric composition
described above is the direction they are being combined towards.

## Summary

Fluxor composes embedded and distributed systems from
capability-bearing modules in deterministic event-driven graphs.
Physical device boundaries, host environments, and transport
protocols become implementation details rather than the primary
architecture.
