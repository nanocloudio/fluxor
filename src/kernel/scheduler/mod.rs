...
pub struct SchedulerState {
    ...
    module_next_due_us: [u64; MAX_MODULES],
    ...
    /// Set from the pod's FXPD attestation flag at admission.
    #[cfg_attr(
        not(feature = "multitenant"),
        allow(dead_code, reason = "read only by the multi-graph runner")
    )]
    module_idle_safe: [bool; MAX_MODULES],
...
/// the pod's FXPD flag at admission). Only an all-idle-safe graph may be fully
...
/// admission-time application of a pod's FXPD idle-safe attestation. Bounded
...
#[cfg(feature = "multitenant")]
pub fn set_owner_idle_safe(owner: crate::kernel::owner::OwnerHandle, idle_safe: bool) {
...
/// Boot path: admit every resident pod declared in the config's `[FXPD]`
/// post-body section (RFC adaptive_tick_extra §7 — the `combine <two-graph.yaml>`
/// / `pods:` mechanism) as a workload owner via `apply_add`, then finalize the
...
/// `finalize_resident_graphs`). No-op when there is no pod section (single-graph
/// config, byte-identical boot) or on non-multitenant builds.
pub fn admit_resident_pods_from_config() {
    #[cfg(feature = "multitenant")]
    {
        // SAFETY: boot context — single-threaded, sole reader of STATIC_CONFIG.
        let cfg = unsafe { static_config() };
        let ptr = cfg.resident_pod_section;
        let len = cfg.resident_pod_section_len;
        ...
        const SCRATCH_LEN: usize = crate::kernel::config::MAX_POD_SECTION_BYTES;
        static mut POD_SCRATCH: [u8; SCRATCH_LEN] = [0u8; SCRATCH_LEN];
        // Atomic admission: a partial admit (some pods live, others rejected)
        // would contradict the "all pods" contract and leave the runtime in a
        // half-built state. Record each admitted owner's handle; if ANY pod fails,
        ...
        let mut handles = [crate::kernel::owner::OwnerHandle {
            slot: 0,
            generation: 0,
        }; MAX_GRAPH_PACERS];
        ...
        let mut off = 12usize;
        for _ in 0..count {
            // Per-pod frame: blob_len(4) + flags(1) + blob. flags bit0 = idle-safe
            // attestation (the operator/tooling asserts the pod is demand-driven).
            if off + 5 > len {
                log::error!("[multigraph] pod section truncated mid-frame; rolling back");
                failed = true;
                break;
            }
            ...
            off += 4;
            let pod_flags = section[off];
            off += 1;
            let pod_idle_safe = (pod_flags & 0x01) != 0;
            if blob_len == 0 || off + blob_len > len || blob_len > SCRATCH_LEN {
                log::error!("[multigraph] pod blob len {blob_len} out of range; rolling back");
                failed = true;
                break;
            }
            // SAFETY: scheduler-thread-exclusive boot context; bounded copy.
            let scratch = unsafe { &mut *core::ptr::addr_of_mut!(POD_SCRATCH) };
            ...
            if rc >= 0 {
                if admitted < MAX_GRAPH_PACERS {
                    handles[admitted] = crate::kernel::owner::OwnerHandle {
                        slot: u16::from_le_bytes([scratch[0], scratch[1]]),
                        generation: u32::from_le_bytes([
                            scratch[2], scratch[3], scratch[4], scratch[5],
                        ]),
                    };
                    idle_flags[admitted] = pod_idle_safe;
                }
                admitted += 1;
            } else {
                log::error!("[multigraph] resident pod admit failed rc={rc}; rolling back");
                failed = true;
                break;
            }
        }
        if failed {
            ...
            log::error!(
                "[multigraph] resident pod admission aborted; rolled back {admitted} pod(s) — \
                 none active (single-graph boot)"
            );
            return;
        }
        if admitted > 0 {
            // Apply each pod's idle-safe attestation to its admitted modules
            // BEFORE finalize (which rebuilds the per-graph `idle_safe` from the
            // module flags). Unattested pods stay fail-closed (backstop cadence).
            for i in 0..admitted.min(MAX_GRAPH_PACERS) {
                set_owner_idle_safe(handles[i], idle_flags[i]);
            }
            log::info!("[multigraph] admitted {admitted} resident pod(s) from config");
...
fn multi_graph_runner(modules: &mut [ModuleSlot; MAX_MODULES], domain: usize) -> (StepResult, u32) {
    ...
    for gi in 0..rc {
        ...
        let woken = crate::kernel::event::take_wake_in_mask(&mask);
