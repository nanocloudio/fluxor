...
pub unsafe fn isr_tier1b_handler() {
    ...
    let count = ISR_COUNT as usize;
    let slots = &raw mut ISR_SLOTS;
    let saved_module = crate::kernel::exec::scheduler::current_module_index();
    for slot in (*slots).iter_mut().take(count) {
        if !slot.active {
            continue;
        }

        crate::kernel::exec::scheduler::set_current_module(slot.module_index as usize);
    ...
    crate::kernel::exec::scheduler::set_current_module(saved_module);
}
...
pub fn isr_metrics_dispatch(arg: *mut u8, arg_len: usize) -> i32 {
    ...
    match query_metrics(tier, slot) {
        ...
        None => crate::kernel::sys::errno::EINVAL,
    }
}
