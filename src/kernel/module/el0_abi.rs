...
//! BCM2712 gateway (`platform::mmu`'s `el0` submodule) builds the region list
...
/// Append one telemetry record to the kernel ring. The record pointer travels
/// in `x2` (`ptr`) and its length in `x3` (`len`); `x1` (`chan`) is unused.
/// Identity is the kernel-known EL0 `module_idx` — an isolated module cannot
/// forge it. Without this op, EL0-isolated modules would lose telemetry
/// emission while the observability lint still mandates it.
