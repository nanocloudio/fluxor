...
use crate::fnv1a32;
use crate::kernel::boot::config::read_layout;
use crate::kernel::sys::hal;
use crate::modules::{Module, StepOutcome};

/// Read the root-of-trust Ed25519 signing pubkey provisioned at BUILD TIME via
/// the `FLUXOR_SIGNING_PUBKEY_HEX` environment variable (64 hex chars → 32
/// bytes) into `out`. Returns true iff a valid key is provisioned; false (with
/// `enforce_signatures`, the caller then refuses every module) otherwise.
///
        ...
        // tampered manifest header explicitly under enforcement.
        {
            ...
            }
            {
                ...
                if has_signature {
                    ...
                    if signing_pubkey_from_build_env(&mut pubkey) {
                        if !crate::kernel::security::crypto::ed25519::verify(&pubkey, &hash, &sig) {
        ...
        match invoke_new(self.new_fn, &*self.syscalls, &args)? {
            NewStatus::Ready => {
                ...
                Ok(Some(DynamicModule {
                    ...
                    isolated: crate::kernel::exec::scheduler::module_is_isolated(
...
impl DynamicModule {
    ...
    pub unsafe fn start_new(
        ...
        let inst_idx = crate::kernel::exec::scheduler::current_module_index();
        crate::kernel::exec::scheduler::set_instantiation_state(inst_idx, state_ptr);
        ...
        if let Ok(arena_addr) = module.get_export_addr(export_hashes::MODULE_ARENA_SIZE) {
            ...
            if arena_size > 0 {
                ...
                match arena_alloc {
                    ...
                    Err(e) => {
                        log::error!(
                            "[loader] {}: heap arena alloc failed need={} (state used={}, cap={}); \
                             bump STATE_ARENA_SIZE in abi::config::kernel or shrink the module's \
...
impl Module for DynamicModule {
    fn step(&mut self) -> Result<StepOutcome, i32> {
        ...
        let result = if self.isolated {
            // SAFETY: step_fn/state_ptr validated at construction; on the
            // EL0 platform the scheduler set `current_module_index()` before
...
pub fn resolve_export_for_module(module_idx: usize, export_hash: u32) -> Option<usize> {
    let (code_base, export_table, export_count) =
        crate::kernel::exec::scheduler::get_module_exports(module_idx);
    if export_table.is_null() || export_count == 0 || code_base == 0 {
        return None;
    }
    let (_, code_size) = crate::kernel::exec::scheduler::module_code_region(module_idx);
    if code_size == 0 {
        return None;
    }
