...
//! - FRAME_BEGIN [0x01] [clear_r:f32, clear_g:f32, clear_b:f32]
...
fn gpu_step(state: *mut u8) -> i32 {
    unsafe {
        ...
        // Log first ready
        if st.frames == 0 && STEP_COUNT < 10 {
            log_msg(b"[gpu] ready, reading channel");
        }
        ...
        while st.cmd_offset < st.cmd_len {
            ...
            match cmd {
                CMD_FRAME_BEGIN => {
                    // 12 bytes: clear color (3 floats)
                    if st.cmd_offset + 12 > st.cmd_len {
                        st.cmd_offset -= 1; // Rewind
                        break;
                    }
                    if st.frames == 0 {
                        log_msg(b"[gpu] FRAME_BEGIN");
                    }
