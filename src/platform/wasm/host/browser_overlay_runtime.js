...
(function (root) {
  ...
  function mount(opts) {
    ...
    // A setting checkbox (Amendment A) — distinct from `toggle`: its
    ...
    // Amendment A §A.3). Options come from `opts.lists[spec.list]`;
    ...
    function relayout(viewport) {
      ...
      const coarse = viewport ? !!viewport.coarse
        : !!(root.matchMedia && root.matchMedia('(pointer: coarse)').matches);
      setOrientation(computeOrientation(w, h, coarse));
    }
