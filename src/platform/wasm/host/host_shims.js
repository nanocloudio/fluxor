...
`;
  ...
  function buildHostImports(opts) {
    ...
    // ── WebCodecs video bridge (wasm_browser_video_codec) ────────────
    //
    // Direct playback of H.264/H.265 samples demuxed by the kernel.
    // The kernel passes the Matroska CodecPrivate (avcC/hvcC) once,
    // then whole length-prefixed samples ("avc"/"hevc" bitstream
    // format — no Annex B anywhere). Decoded VideoFrames are PTS-paced
    // against a wall-clock base and drawn straight onto the page
    // canvas; the browser handles any HDR→SDR conversion when
    // compositing (BT.2020/PQ VideoFrames onto an SDR 2D canvas).
    const video = {
      dec: null, fatal: false, frames: [], presenting: false,
      basePts: null, baseWall: null, w: 0, h: 0, presented: 0,
      pendingChunks: null,
      // Diagnostics / wedge recovery
      chunks: 0, lastChunkTs: 0, needKey: false,
      hbTimer: null, lastPresented: -1, stuckSince: 0,
      lastConfig: null,
    };

    // Periodic pipeline heartbeat + wedge detection. If no frame
    // presents for 10 s while work is queued, the decoder has wedged
    // (seen with some hw decoders on long streams) — reset it and
    // resume at the next keyframe rather than freezing forever.
    function videoHeartbeat() {
      if (!video.dec) return;
      const q = video.dec.decodeQueueSize;
      console.log(`[video] hb state=${video.dec.state} decq=${q} ` +
                  `frames=${video.frames.length} presented=${video.presented} ` +
                  `chunks=${video.chunks} lastTs=${video.lastChunkTs}ms`);
      const queued = q + video.frames.length;
      if (video.presented === video.lastPresented && queued > 0) {
        video.stuckSince += 5;
        if (video.stuckSince >= 10 && video.dec.state === 'configured') {
          console.warn(`[video] pipeline stuck ${video.stuckSince}s ` +
                       `(decq=${q} frames=${video.frames.length}) — resetting decoder`);
          try {
            video.dec.reset();
            if (video.lastConfig) video.dec.configure(video.lastConfig);
            video.frames.forEach((f) => f.close());
            video.frames = [];
            video.basePts = null; // rebase pacing at next frame
            video.needKey = true; // drop deltas until a keyframe
            video.stuckSince = 0;
          } catch (err) {
            console.error(`[video] reset failed: ${err.message}`);
            video.fatal = true;
            videoBanner(`Decoder wedged and reset failed: ${err.message}`);
          }
        }
      } else {
        video.stuckSince = 0;
      }
      video.lastPresented = video.presented;
    }

    // Visible failure reporting — a blank canvas tells the user
    // nothing; put the decoder verdict on the page.
    function videoBanner(text) {
      console.error(`[video] ${text}`);
      try {
        const el = document.createElement('div');
        el.textContent = `[video] ${text}`;
        el.style.cssText = 'color:#f66;font:14px monospace;padding:12px;white-space:pre-wrap;';
        (canvasContainer || document.body).appendChild(el);
      } catch (_) {}
    }

    function hexByte(b) { return b.toString(16).padStart(2, '0'); }

    // RFC 6381 codec string from an avcC record.
    function avcCodecString(d) {
      return `avc1.${hexByte(d[1])}${hexByte(d[2])}${hexByte(d[3])}`;
    }

    // RFC 6381 / ISO 14496-15 codec string from an hvcC record.
    function hevcCodecString(d) {
      const b1 = d[1];
      const space = (b1 >> 6) & 3;
      const tier = (b1 >> 5) & 1;
      const idc = b1 & 0x1f;
      // 32 compatibility flag bits, bit-reversed for the string.
      let compat = (d[2] << 24) | (d[3] << 16) | (d[4] << 8) | d[5];
      let rev = 0 >>> 0;
      for (let i = 0; i < 32; i++) { rev = (rev << 1) | ((compat >>> i) & 1); }
      rev = rev >>> 0;
      const constraints = [d[6], d[7], d[8], d[9], d[10], d[11]];
      while (constraints.length > 1 && constraints[constraints.length - 1] === 0) {
        constraints.pop();
      }
      const level = d[12];
      return `hvc1.${['', 'A', 'B', 'C'][space]}${idc}.${rev.toString(16).toUpperCase()}` +
             `.${tier ? 'H' : 'L'}${level}` +
             constraints.map((c) => '.' + hexByte(c).toUpperCase()).join('');
    }

    function videoPresentLoop() {
      if (video.presenting) return;
      video.presenting = true;
      const tick = () => {
        if (video.frames.length === 0) { video.presenting = false; return; }
        const f = video.frames[0];
        const ptsMs = f.timestamp / 1000;
        const now = performance.now();
        if (video.basePts === null) {
          video.basePts = ptsMs;
          video.baseWall = now;
        }
        const due = video.baseWall + (ptsMs - video.basePts);
        if (now + 1 >= due) {
          video.frames.shift();
          try {
            ensureCanvas(video.w, video.h);
            canvasCtx.drawImage(f, 0, 0, video.w, video.h);
            onCanvasFrame(video.w, video.h);
          } catch (err) {
            console.error(`video present: ${err.message}`);
          }
          f.close();
          video.presented++;
          setTimeout(tick, 0);
        } else {
          setTimeout(tick, Math.min(due - now, 250));
        }
      };
      setTimeout(tick, 0);
    }
    ...
    const videoShim = {
      host_video_config: (kind, descPtr, descLen, width, height) => {
        try {
          if (typeof VideoDecoder === 'undefined') {
            videoBanner('WebCodecs is unavailable in this browser / context. ' +
                        'Use Chrome/Edge/Safari over localhost or HTTPS.');
            return -1;
          }
          const desc = new Uint8Array(
            getKernel().exports.memory.buffer.slice(descPtr, descPtr + descLen)
          );
          // Candidate codec strings, most-specific first. Browsers
          // disagree on how much of the RFC 6381 tail they want, and
          // on hvc1 (samples carry parameter sets in `description`)
          // vs hev1; probe until one sticks.
          let candidates;
          if (kind === 2) {
            const full = hevcCodecString(desc);
            const noConstraints = full.split('.').slice(0, 4).join('.');
            candidates = [full, noConstraints,
                          full.replace('hvc1', 'hev1'),
                          noConstraints.replace('hvc1', 'hev1')];
          } else {
            candidates = [avcCodecString(desc)];
          }
          if (video.dec) { try { video.dec.close(); } catch (_) {} }
          video.dec = null;
          video.fatal = false;
          video.frames.forEach((f) => f.close());
          video.frames = [];
          video.pendingChunks = [];
          video.basePts = null;
          video.w = width; video.h = height; video.presented = 0;
          ...
          (async () => {
            ...
            for (const codec of candidates) {
              try {
                const res = await VideoDecoder.isConfigSupported({
                  ...
                  codedWidth: width || undefined, codedHeight: height || undefined,
                });
                console.log(`[video] probe ${codec} -> ${res.supported}`);
                if (res.supported) { chosen = codec; break; }
              } catch (err) {
                console.log(`[video] probe ${codec} -> throw: ${err.message}`);
              }
            }
            if (!chosen) {
              video.fatal = true;
              videoBanner(`This browser's WebCodecs cannot decode this stream ` +
                          `(tried: ${candidates.join(', ')}). HEVC needs ` +
                          `Chrome/Edge/Safari with platform HEVC support.`);
              return;
            }
            console.log(`[video] configuring VideoDecoder codec=${chosen} ${width}x${height}`);
            const dec = new VideoDecoder({
              output: (frame) => { video.frames.push(frame); videoPresentLoop(); },
              error: (e) => {
                console.error(`[video] decoder error: ${e.message}`);
                video.fatal = true;
                videoBanner(`Decoder error: ${e.message}`);
              },
            });
            const cfg = {
              codec: chosen,
            ...
            dec.configure(cfg);
            video.dec = dec;
            video.lastConfig = cfg;
            if (!video.hbTimer) video.hbTimer = setInterval(videoHeartbeat, 5000);
            // Replay samples that arrived while probing.
            const backlog = video.pendingChunks;
            video.pendingChunks = null;
            for (const c of backlog) {
              try { dec.decode(c); } catch (err) {
                console.error(`[video] backlog decode: ${err.message}`);
                video.fatal = true;
                break;
              }
            }
          ...
          console.error(`host_video_config threw: ${err.message}`);
          return -1;
        }
      },
      host_video_chunk: (tsMs, keyframe, ptr, len) => {
        if (video.fatal) return -2;
        try {
          // Copy out of kernel memory — the chunk outlives this call
          // and the wasm heap can move on memory.grow.
          const data = new Uint8Array(
            getKernel().exports.memory.buffer.slice(ptr, ptr + len)
          );
          if (video.needKey && !keyframe) {
            return 0; // post-reset: skip deltas until the next keyframe
          }
          if (video.needKey && keyframe) video.needKey = false;
          const chunk = new EncodedVideoChunk({
            type: keyframe ? 'key' : 'delta',
            timestamp: (tsMs >>> 0) * 1000,
            data,
          });
          video.chunks++;
          video.lastChunkTs = tsMs >>> 0;
          if (!video.dec) {
            // Codec probe still in flight — buffer (bounded; the
            // kernel's queue watermark keeps this small).
            if (video.pendingChunks) video.pendingChunks.push(chunk);
            return 0;
          }
          if (video.dec.state === 'closed') return -1;
          video.dec.decode(chunk);
          return 0;
        } catch (err) {
          console.error(`host_video_chunk threw: ${err.message}`);
          video.fatal = true;
          return -2;
        }
      },
      host_video_status: () => {
        if (video.fatal) return -2;
        const q = video.dec ? video.dec.decodeQueueSize : 0;
        const backlog = video.pendingChunks ? video.pendingChunks.length : 0;
        return q + backlog + video.frames.length;
      },
      host_video_flush: () => {
        if (video.dec && video.dec.state === 'configured') {
          video.dec.flush().catch(() => {});
        }
        return 0;
      },
    };
