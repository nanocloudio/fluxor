...
//
// SPI, I2C, UART, and ADC providers have been moved to PIC modules that register
// via REGISTER_PROVIDER. The raw register bridges remain in rp_system_extension_dispatch.
...
unsafe fn rp_system_extension_dispatch(_handle: i32, opcode: u32, arg: *mut u8, arg_len: usize) -> i32 {
    ...
    match opcode {
        ...
        dev_system::DMA_FD_START => {
            ...
            dma_fd_start(_handle, read_addr, write_addr, count, dreq, flags)
        }
        dev_system::DMA_FD_RESTART => {
            if arg.is_null() || arg_len < 8 { return E_INVAL; }
            let read_addr = u32::from_le_bytes([*arg, *arg.add(1), *arg.add(2), *arg.add(3)]);
            let count = u32::from_le_bytes([*arg.add(4), *arg.add(5), *arg.add(6), *arg.add(7)]);
            dma_fd_restart(_handle, read_addr, count)
        }
        ...
        dev_system::DMA_FD_QUEUE => {
            ...
            let read_addr = u32::from_le_bytes([*arg, *arg.add(1), *arg.add(2), *arg.add(3)]);
            let count = u32::from_le_bytes([*arg.add(4), *arg.add(5), *arg.add(6), *arg.add(7)]);
            dma_fd_queue(_handle, read_addr, count)
        }
        // ── SYS_CLOCK_HZ ──
...
// ============================================================================
// RP dev_query extension (GPIO + SYS_CLOCK_HZ for dev_query path)
// ============================================================================

unsafe fn rp_dev_query_extension(handle: i32, key: u32, out: *mut u8, out_len: usize) -> i32 {
    use crate::abi::{dev_class, dev_gpio, dev_system};
...
fn init_rp_providers() {
    ...
    // PIO, SPI, I2C, UART, ADC providers are registered by PIC modules
    // via REGISTER_PROVIDER at module init time.
