...
// # Channels
//
// - in[0]: From mesh — framed messages to PUBLISH
// - out[0]: To mesh — received PUBLISH payloads
//
...
// # State Machine
//
//   Init -> SocketOpen -> Connecting -> WaitConnect -> MqttConnect ->
...
// - DISCONNECT (0xE0): 2 bytes (not used — we just close socket)
...
// ============================================================================
// Socket send helper (partial send tracking)
// ============================================================================

/// Try to send tx_buf[tx_sent..tx_len]. Returns true if all sent.
unsafe fn flush_tx(s: &mut MqttState) -> bool {
    ...
    let rc = (sys.dev_call)(
        s.socket_handle,
        DEV_SOCKET_SEND,
        s.tx_buf.as_mut_ptr().add(s.tx_sent as usize),
        remaining,
    );
    if rc > 0 {
        s.tx_sent += rc as u16;
                        ...
                        if total_frame <= CHAN_BUF_SIZE {
                            ...
                            *cb = topic_len as u8;
                            ...
                            if poll > 0 && ((poll as u8) & POLL_OUT) != 0 {
                                (sys.channel_write)(s.out_chan, cb, total_frame);
                            }
...
/// Read from socket into rx_buf, process complete packets.
unsafe fn handle_rx(s: &mut MqttState) {
    ...
    if space == 0 {
        ...
        if consumed > 0 {
            ...
            // Stuck — can't parse and no space. Reset rx.
            s.rx_have = 0;
        }
        return;
    }
    ...
    let rc = (sys.dev_call)(
        s.socket_handle,
        DEV_SOCKET_RECV,
        s.rx_buf.as_mut_ptr().add(s.rx_have as usize),
        space,
    );
    ...
    } else if rc < 0 && rc != E_AGAIN {
        // Socket error
        log_err(s, b"[mqtt] recv error");
        s.phase = MqttPhase::Reconnect;
    }
}
...
/// Check in_chan for messages to publish.
unsafe fn handle_tx_from_channel(s: &mut MqttState) {
    // Don't read channel if we still have unsent socket data
    if s.tx_sent < s.tx_len {
        if !flush_tx(s) {
            return;
        }
...
/// Check socket health.
unsafe fn check_socket_health(s: &mut MqttState) {
    let sys = &*s.syscalls;
    let mut poll_arg = [POLL_HUP | POLL_ERR];
    let poll = (sys.dev_call)(s.socket_handle, DEV_SOCKET_POLL, poll_arg.as_mut_ptr(), 1);
    if poll > 0 {
        let flags = poll as u8;
        if (flags & POLL_HUP) != 0 || (flags & POLL_ERR) != 0 {
            log_err(s, b"[mqtt] socket lost");
            s.phase = MqttPhase::Reconnect;
        }
    }
}
...
unsafe fn enter_reconnect(s: &mut MqttState) {
    ...
    if s.socket_handle >= 0 {
        let sys = &*s.syscalls;
        (sys.dev_call)(s.socket_handle, DEV_SOCKET_CLOSE, core::ptr::null_mut(), 0);
        s.socket_handle = -1;
    }
...
pub extern "C" fn module_step(state: *mut u8) -> i32 {
    unsafe {
        ...
        loop {
            match s.phase {
                MqttPhase::Init => {
                    log_msg(s, b"[mqtt] opening socket");
                    s.phase = MqttPhase::SocketOpen;
                    continue;
                }

                MqttPhase::SocketOpen => {
                    let mut sock_arg = [SOCK_TYPE_STREAM];
                    let handle = (sys.dev_call)(-1, DEV_SOCKET_OPEN, sock_arg.as_mut_ptr(), 1);
                    if handle < 0 {
                        log_err(s, b"[mqtt] socket_open fail");
                        enter_reconnect(s);
                        return 0;
                    }
                ...
                MqttPhase::Connecting => {
                    let addr = ChannelAddr::new(s.broker_ip, s.broker_port);
                    let rc = (sys.dev_call)(s.socket_handle, DEV_SOCKET_CONNECT, &addr as *const _ as *mut u8, core::mem::size_of::<ChannelAddr>());
                    if rc < 0 && rc != E_INPROGRESS {
                        log_err(s, b"[mqtt] connect fail");
                        enter_reconnect(s);
                        return 0;
                    }
...
pub extern "C" fn module_channel_hints(out: *mut u8, max_len: usize) -> i32 {
    let hints = [
        ChannelHint { port_type: 0, port_index: 0, buffer_size: 256 }, // in[0]: from mesh
        ChannelHint { port_type: 1, port_index: 0, buffer_size: 256 }, // out[0]: to mesh
    ];
