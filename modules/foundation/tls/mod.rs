...
/// Per-tick work is O(MAX_SESSIONS), but for non-handshaking (Ready) sessions
/// the per-session step is just a couple of cheap state checks — the real work
/// is message-driven, not per-session — so the dominant cost of raising this is
/// memory (~13 KB/session), not tick overhead. A Ready keep-alive connection
/// holds its session slot, so MAX_SESSIONS is the hard cap on *concurrent* TLS
/// connections: at 16 it capped achievable throughput at ~16 conns × ~600 rps
/// ≈ 10k rps even though core 0 had CPU headroom. 64 lifts the concurrency
...
struct TlsState {
    ...
    // refill it is permanently empty after the first `MAX_SESSIONS`
    // accepts, and every subsequent handshake pays the ~900 µs synchronous
    // `ecdh_keygen` on the hot accept path (baseline: pool_hit=16,
    // fallback_keygen=617). This regenerates one consumed slot at a time,
    ...
    refill_slot: usize,
    /// Private scalar for the in-flight slot, staged until `k·G` completes.
    refill_priv: [u8; 32],
    /// Resumable `k·G` ladder for the in-flight slot.
    refill_state: ScalarMulState,
    /// Lifetime count of slots regenerated by the background refill.
    /// Emitted on the `[tls] hb` line to confirm the pool stays warm.
    ecdh_pool_refill: u32,
    ...
    //
    // Byte counters and step histograms say how much work the module did.
    // Neither says whether it had more work available and stopped anyway,
    // which is what distinguishes "this stage is the gate" from "this stage
    // is starved waiting on someone else". A pipelined graph can sit well
    // under its CPU budget with every drop counter at zero and still be
    // rate-limited by a per-tick advancement rule; these two name where.
    /// Inbound frames drained this window (both sides summed).
    adv_items: u32,
    /// Steps that ended with `cipher_in` or `clear_in` still readable —
    /// tls had more to do and stopped. Sustained non-zero means tls is the
    /// gate; zero means it is keeping up and the queue is elsewhere.
    pend_steps: u32,
}
...
///
/// Without a budget each phase would take one frame per tick however many the
/// producer had queued, which caps a pipelined graph at one frame per side per
/// tick regardless of how much headroom the domain has. Burst does not
/// compensate: `MAX_BURST_STEPS` re-steps the same module rather than
/// advancing a request to the next one.
///
/// 8, deliberately below `ip`'s 32 RX budget. A larger drain lengthens every
/// step, which costs the latency-bound paths — single-connection keepalive
/// throughput is 1/latency and handshakes are round-trip bound — and buys
/// nothing once the queue is already being cleared each tick. This is a
/// throughput/latency trade: measure it under concurrency, because
/// single-connection latency is the metric a larger budget damages first.
const TLS_INBOUND_DRAIN_BUDGET: u32 = 8;
...
pub unsafe extern "C" fn module_step(state: *mut u8) -> i32 {
    ...
    // Keep the ephemeral ECDH pool warm: regenerate one consumed slot per
    // tick in the background so fresh accepts hit the pool (a memcpy) instead
    ...
    while ci_drained < TLS_INBOUND_DRAIN_BUDGET {
        ...
        // possible this tick, and continuing would spin on a ready poll.
        if msg_type == 0 {
            break;
        }
        ci_drained += 1;
        s.adv_items = s.adv_items.wrapping_add(1);
        did_work = true;
    ...
    // Did this step leave work on the table? Either inbound side still
    // readable means we stopped with frames pending — see `pend_steps`.
    {
        let ci = (sys.channel_poll)(s.cipher_in, POLL_IN);
        let cl = (sys.channel_poll)(s.clear_in, POLL_IN);
        let ci_ready = ci > 0 && (ci as u32 & POLL_IN) != 0;
        let cl_ready = cl > 0 && (cl as u32 & POLL_IN) != 0;
        if ci_ready || cl_ready {
            s.pend_steps = s.pend_steps.wrapping_add(1);
        }
    }
...
/// private key).
/// Background ECDH-pool refill — call once per `module_step`.
///
/// Regenerates one consumed pool slot at a time. A slot is "consumed" when
/// `eph_used[k]` is true (handed to a handshake by `assign_fresh_ecdh_key`).
/// Each tick:
///   * if no refill is in flight, pick the first consumed slot, draw a fresh
///     scalar, and start its `k·G` ladder (no slot state is touched yet);
///   * if a refill is in flight, advance the ladder by `ecdh_bits_per_step`
///     bits; on completion, install the new keypair into the slot and clear
///     `eph_used` so `assign_fresh_ecdh_key` can hand it out again.
///
/// The per-tick cost is one `ScalarMulState::step()` — the same bounded work
/// the handshake's own ECDH already pays — so it fits the tick budget. The
/// in-flight slot keeps `eph_used == true` until the keypair is valid, so the
/// assign-side free-slot scan never races a half-built slot. When the pool is
/// fully stocked (`eph_used` all false) this is a single O(MAX_SESSIONS) scan
/// and returns.
unsafe fn pump_ecdh_refill(s: &mut TlsState) {
    if s.refill_active {
        if !s.refill_state.step() {
            return; // ladder not finished this tick
        }
        // `k·G` complete — install into the slot. A None finalise means the
        // point at infinity, which the non-zero scalar clamp precludes; on
        // the impossible chance it happens, drop the attempt and let the next
        // tick restart a fresh one rather than install a bad key.
        if let Some(pub_key) = ecdh_keygen_finalise(&s.refill_state) {
            let slot = s.refill_slot;
            s.eph_private[slot] = s.refill_priv;
            s.eph_public[slot] = pub_key;
            s.eph_used[slot] = false;
            s.ecdh_pool_refill = s.ecdh_pool_refill.wrapping_add(1);
        }
        // Scrub the staging scalar + ladder secret regardless of outcome.
        let mut j = 0;
        while j < 32 {
            core::ptr::write_volatile(&mut s.refill_priv[j], 0);
            j += 1;
        }
        s.refill_state.zeroise_scalar();
        s.refill_active = false;
        return;
    }

    // No refill in flight — find a consumed slot to regenerate.
    ...
    let slot = match slot {
        Some(idx) => idx,
        None => return, // pool fully stocked
    };

    let sys = &*s.syscalls;
    let mut random = [0u8; 32];
    if dev_csprng_fill(sys, random.as_mut_ptr(), 32) < 0 {
        return; // CSPRNG hiccup — retry next tick, slot stays consumed
    }
    // Same per-step chunking the handshake ECDH uses. `>= 256` (run-to-
    // completion / vault mode) maps to 0 = finish in one `step()`.
    let bps: u8 = if s.ecdh_bits_per_step == 0 || s.ecdh_bits_per_step >= 256 {
        0
    } else {
        s.ecdh_bits_per_step as u8
    };
    let (priv_key, state) = ecdh_keygen_init(&random, bps);
    s.refill_priv = priv_key;
    s.refill_state = state;
    s.refill_slot = slot;
    s.refill_active = true;
    let mut j = 0;
    while j < 32 {
        core::ptr::write_volatile(random.as_mut_ptr().add(j), 0);
        j += 1;
    }
}
