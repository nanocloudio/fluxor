...
pub extern "C" fn module_new(
    ...
    unsafe {
        ...
        // Generative defaults (all off — playback stays deterministic)
        ...
        // Params arrive as TLV, tagged by the schema in `params_def`
        // (`0xFE 0x01` header). Absent or unrecognised params take the
        // schema's declared defaults — `sample_rate` is applied first, so
        // the `step_ms` closure derives `step_frames` from it.
        let is_tlv =
            !params.is_null() && params_len >= 4 && *params == 0xFE && *params.add(1) == 0x01;
        if is_tlv {
            params_def::parse_tlv(seq, params, params_len);
        } else {
            params_def::set_defaults(seq);
        }
        ...
        if seq.preset_count == 0 {
            dev_log(
                &*seq.syscalls,
                1,
                b"[seq] error: no presets provided\0".as_ptr(),
                31,
            );
            return -10;
        }
    ...
    unsafe {
                    ...
                    match ty {
                        MSG_STATUS => {
                            ...
                            if len >= 2 {
                                ...
                                if seq.preset_count > 0
                                    && (index as usize) < seq.preset_count as usize
                                {
                        ...
                        MSG_PREV => {
                            if seq.preset_count > 0 {
                                let prev =
                                    (seq.current_preset + seq.preset_count - 1) % seq.preset_count;
        ...
        if seq.started != 0 && seq.current_value == seq.last_value_sent && seq.ratchet_pending == 0
        {
            ...
            if seq.mode != SeqMode::OneShot && seq.position == 0 && seq.started != 0 {
                ...
                if seq.auto_advance_preset != 0 && seq.preset_count > 1 {
                    ...
                    let next = if (next as usize) >= seq.preset_count as usize {
                        0
                    } else {
                        next
                    };
        ...
        if seq.ratchet_pending > 0 {
            ...
            if poll > 0 && ((poll as u32) & POLL_OUT) != 0 {
                ...
                let count = if seq.ratchet_count == 0 {
                    1
                } else {
                    seq.ratchet_count
                };
                ...
                let mut abs_frame = if seq.hw_synced != 0 {
                    let abs = (seq.hw_frame_base as u32)
                        .wrapping_add(seq.ratchet_base_frame.wrapping_add(offset));
        ...
        if seq.ratchet_pending == 0
            && (seq.current_value != seq.last_value_sent || seq.retrigger != 0)
        {
