//! Object Bank PIC Module — storage.namespace/object playlist player.
//!
//! A sibling of [`foundation/bank`](../bank): identical navigation,
//! FMP-command, and streaming machinery, but its backend is swapped from
//! the `fs` contract to the **storage** surfaces — it enumerates a
//! prefix via `storage.namespace::LIST` and streams each entry via
...
struct BankState {
    ...
    /// storage.object read cursor for the currently-open handle — the
    /// next `OBJ_RANGE_GET` offset. Reset to 0 on each open.
    obj_offset: u64,
...
unsafe fn fs_open_index(s: &mut BankState, idx: u16) -> bool {
    ...
    let fd = (s.sys().provider_call)(-1, OBJ_GET, path.as_mut_ptr(), n);
    if fd < 0 {
        return false;
    }
    s.fs_fd = fd;
...
unsafe fn fs_scan_dir(s: &mut BankState) {
    ...
    loop {
        ...
        if !saw_cursor || next_cursor_len == 0 { break; }
...
pub extern "C" fn module_step(state: *mut u8) -> i32 {
    unsafe {
        ...
        if s.paused == 0 && s.out_chans[0] >= 0 && s.path_count > 0 {
            ...
            // storage.object RANGE_GET at the current cursor into s.buf.
            // arg = [offset:u64 LE][length:u32 LE][out_ptr:u64 LE]; the
            // provider returns the byte count (n>0), EAGAIN (-11) while
            // pending, or 0 once the window is exhausted at the tail.
