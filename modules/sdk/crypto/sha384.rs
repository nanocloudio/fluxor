...
impl Sha384 {
    ...
    pub fn finalize(mut self) -> [u8; 48] {
        ...
        unsafe {
            core::ptr::copy_nonoverlapping(
                len_hi.to_be_bytes().as_ptr(),
                self.buf.as_mut_ptr().add(112),
                8,
            );
            core::ptr::copy_nonoverlapping(
                len_lo.to_be_bytes().as_ptr(),
                self.buf.as_mut_ptr().add(120),
                8,
            );
        }
        let block = self.buf;
