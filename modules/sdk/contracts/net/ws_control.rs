// Contract: ws_control — the records that drive a WebSocket connector
// from outside it, and the number of links one carries.
//
// Layer: contracts/net (public, stable).
//
// A consumer holding several links addresses them by index, and that index
// rides in `WsFrame`'s `conn` field, so the messages themselves need
// nothing from here. What a frame has no room to say is here instead:
// which resource to open, and what became of a link.
//
// Every end of this seam mounts this file — the consumer that opens links,
// the connector that performs them on a socket, and the connector that
// performs them through a browser — so the count and the layouts are
// written once and cannot be restated. That is why it lives beside
// `ws_frame` rather than with either connector: all three depend on fluxor
