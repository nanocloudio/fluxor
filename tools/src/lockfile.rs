...
/// transaction; released on drop. The guard file (`target/fluxor/.lockfile-guard`)
/// is separate from the lockfile so locking never truncates the data, and lives
/// under `target/` so it is build-output, never repo content. Because anything
/// under `target/` may be deleted at any time (`cargo clean`), the lock is only
