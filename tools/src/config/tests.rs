...
#[cfg(test)]
mod otlp_idtable_capacity_tests {
    //! Drift guard: the `otlp_http` id-table bound is declared once, in the
    //! module's `manifest.toml` `[capacities] idtable` table, and consumed
    //! twice — by `build_otlp_id_table_text`, which truncates the injected
    //! table at that size, and by the module itself (`IDTABLE_MAX`, the fixed
    //! array in its arena-allocated state). The module is `no_std` and cannot
    //! read its own manifest at compile time, so that second copy is a
    //! hand-written constant; these tests pin it to the manifest.
    //!
    //! Drift is silent in production: the compiler injects up to ITS bound and
    //! the module truncates at ITS own, at a byte offset rather than a `;`
    //! entry boundary, so every name past the smaller bound resolves to
    //! nothing at the collector while samples keep exporting.
    ...
    fn module_dir() -> PathBuf {
        PathBuf::from(env!("CARGO_MANIFEST_DIR"))
            ...
            .join("modules/foundation/otlp_http")
    }

    fn module_src() -> String {
        std::fs::read_to_string(module_dir().join("mod.rs")).expect("otlp_http/mod.rs missing")
    }

    fn declared_idtable(silicon: &str) -> u32 {
        let manifest = Manifest::from_toml_for_target(&module_dir().join("manifest.toml"), Some(silicon))
            .unwrap_or_else(|e| panic!("otlp_http manifest does not parse for {silicon}: {e}"));
        manifest
            .capacities
            .get("idtable")
            .copied()
            .unwrap_or_else(|| panic!("otlp_http manifest declares no `[capacities] idtable`"))
    }

    /// Every `const IDTABLE_MAX: usize = N;` in the module source, paired with
    /// the `#[cfg(...)]` attribute immediately preceding it (doc comments
    /// skipped). An unguarded declaration yields an empty cfg string, which no
    /// expected arm matches.
    fn idtable_consts(src: &str) -> Vec<(String, u32)> {
        let lines: Vec<&str> = src.lines().collect();
        let mut found = Vec::new();
        for (i, line) in lines.iter().enumerate() {
            let trimmed = line.trim();
            let Some(rest) = trimmed.strip_prefix("const IDTABLE_MAX: usize = ") else {
                continue;
            };
            let value: u32 = rest.trim_end_matches(';').parse().unwrap_or_else(|_| {
                panic!("IDTABLE_MAX is not a plain decimal literal: {trimmed}")
            });
            ...
            for prev in lines[..i].iter().rev() {
                ...
                if p.starts_with("#[cfg(") {
                    cfg = p.to_string();
                }
                break;
            }
            found.push((cfg, value));
        }
        found
    }

    /// `fluxor modules build` compiles the module once per silicon, so a
    /// `target_arch` cfg is the module-side discriminator: bcm2712 builds for
    /// `aarch64-unknown-none`, rp2350 for `thumbv8m.main-none-eabihf`. Each
    /// arm's literal must equal what the manifest resolves for the silicon that
    /// arm covers, or the compiler and the module disagree on the buffer size.
    #[test]
    fn module_constants_match_manifest_capacities() {
        let consts = idtable_consts(&module_src());
        assert_eq!(
            consts.len(),
            2,
            "expected one aarch64-gated and one non-aarch64 IDTABLE_MAX, found {consts:?}"
        );

        for (cfg, silicon) in [
            ("#[cfg(target_arch = \"aarch64\")]", "bcm2712"),
            ("#[cfg(not(target_arch = \"aarch64\"))]", "rp2350"),
        ] {
            let expected = declared_idtable(silicon);
            let (_, value) = consts
                .iter()
                .find(|(c, _)| c == cfg)
                .unwrap_or_else(|| panic!("otlp_http/mod.rs has no `{cfg}` IDTABLE_MAX arm"));
            assert_eq!(
                *value, expected,
                "otlp_http {cfg} IDTABLE_MAX = {value} but manifest \
                 `[capacities] idtable` resolves to {expected} for {silicon}"
            );
        }
    }

    /// The state array is charged to every silicon the module is placed on, so
    /// the small-silicon arm must not inherit the large one: `default` is what
    /// rp2350 — and every unlisted silicon — pays for.
    #[test]
    fn small_silicon_pays_less_than_bcm2712() {
        let rp = declared_idtable("rp2350");
        let bcm = declared_idtable("bcm2712");
        assert!(
            rp < bcm,
            "otlp_http declares hardware_targets bcm2712 + rp2350; an id-table \
             sized for a broker-scale aarch64 graph ({bcm} B) must not be charged \
             to rp2350 arena ({rp} B)"
        );
    }
}
